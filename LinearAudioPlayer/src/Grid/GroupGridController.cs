...
{
    class GroupGridController : SourceGridController
    {
        /*
         * パブリックメンバ
         */
        ...
        public LinearEnum.FilteringMode LastFilteringMode { get; set; }
        ...
        */
        ...
        public override void addItem(ISourceGridItem sgi)
        {
            ...
            //toolTipController.ToolTipTitle = gi.DisplayValue;

            // ID用
            Grid[i, (int)EnuGrid.GROUP] = new Cell(gi.DisplayValue);
            Grid[i, (int) EnuGrid.GROUP].Tag = gi;
            Grid[i, (int)EnuGrid.GROUP].AddController(clickController);
            Grid[i, (int)EnuGrid.GROUP].AddController(toolTipController);
            Grid[i, (int) EnuGrid.GROUP].ToolTipText = gi.DisplayValue;

            if (i == 0)
            {
                Grid[i, (int)EnuGrid.GROUP].Column.Width = 255;
            }
        ...
        public override ISourceGridItem getRowGridItem(int rowNo) {
            ...
            cii.DisplayValue = this.getValue(rowNo, (int)EnuGrid.GROUP);
            cii.Value = Grid[rowNo, (int) EnuGrid.GROUP].Tag.ToString();
